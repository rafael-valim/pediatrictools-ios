...
struct ApgarScoreView: View {
    @State private var scores: [String: Int] = [:]

    private var total: Int {
        ApgarCriteria.all.reduce(0) { $0 + (scores[$1.id] ?? 0) }
    }

    var body: some View {
        Form {
            Section {
                ForEach(ApgarCriteria.all) { criterion in
                    ScoreSelectorRow(
        ...
        .navigationTitle("apgar_title")
        .navigationBarTitleDisplayMode(.inline)
        .modifier(ToolInfoToolbar(toolID: "apgar"))
        .toolbar {
